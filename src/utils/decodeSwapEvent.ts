...
export function decodeLegacySwapEvent(data: `0x${string}`): CoreSwapped {
  let n = BigInt(data);
  ...
  const poolId = `0x${poolIdBigInt.toString(16).padStart(64, "0")}` as `0x${string}`;
  n >>= 256n;

  // locker: address (20 bytes)
  const lockerBigInt = n & MAX_UINT160;
  const locker = `0x${lockerBigInt.toString(16).padStart(40, "0")}` as `0x${string}`;
...
export function decodeSwapEvent(data: `0x${string}`): CoreSwapped {
